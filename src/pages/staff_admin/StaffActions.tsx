...
export function ReplaceTeacherModal({ onClose }: { onClose: () => void }) {
  ...
  const keyOf = (a: { sectionId: string; subjectId: string }) =>
    `${a.sectionId}|${a.subjectId}`;
  ...
  const chosenHomeroom =
    workload.homeroomSectionId && isOn("homeroom")
  ...
  const submit = async () => {
    ...
    setBusy(true);
    await transferWorkload(fromId, toId, {
      assignments: chosenAssignments,
      homeroomSectionId: chosenHomeroom,
    });
    setBusy(false);
    const bits = [
      chosenAssignments.length > 0
        ? `${chosenAssignments.length} ${t("classesMoved")}`
        : null,
      chosenHomeroom
        ? `${t("homeroomRow").toLowerCase()} ${sectionLabel(db, chosenHomeroom)}`
        : null,
    ]
      .filter(Boolean)
      .join(" + ");
    ...
    >
        ...
        >
          {outgoingOptions.map((x) => (
            <option key={x.id} value={x.id}>
              {personName(x)}
              {x.status !== "active" ? ` (${t(STATUS_KEYS[x.status])})` : ""}
            </option>
          ))}
        </select>
      </div>
      ...
      {workload.homeroomSectionId && (
        <label className="flex items-center gap-3 py-2.5 border-b border-line-soft cursor-pointer">
          <input
            type="checkbox"
            className="w-5 h-5 accent-[#F0B400]"
            checked={isOn("homeroom")}
            onChange={() => toggle("homeroom")}
          />
          <span className="flex-1 text-[13.5px]">
            <b>{t("homeroomRow")}</b> —{" "}
            {sectionLabel(db, workload.homeroomSectionId)}
          </span>
        </label>
      )}
      {workload.assignments.map((a) => {
        ...
        return (
          <label
            key={k}
            className="flex items-center gap-3 py-2.5 border-b border-line-soft cursor-pointer"
          >
            <input
              type="checkbox"
              className="w-5 h-5 accent-[#F0B400]"
      ...
      {incomingHomeroom &&
        chosenHomeroom &&
        incomingHomeroom !== chosenHomeroom && (
          <div className="flex gap-2.5 mt-3 p-3 rounded-xl bg-honey/10 border border-honey/30">
            ...
            <p className="text-[12.5px] leading-relaxed">
              {personName(to!)} {t("incomingHasHomeroom")}{" "}
              <b>{sectionLabel(db, incomingHomeroom)}</b>.{" "}
              {t("incomingHomeroomReleased")}
            </p>
          </div>
        )}
      ...
      <div className="flex gap-2.5 mt-4">
        ...
        <button
          ...
          onClick={submit}
          disabled={busy || nothingPicked || !toId}
        >
      ...
      <div className="field">
        ...
        <select
          ...
          onChange={(e) =>
            setForm({
              ...form,
              position: e.target.value as Teacher["position"],
            })
          }
        >
          {(Object.keys(POSITION_KEYS) as (keyof typeof POSITION_KEYS)[]).map(
            (p) => (
        ...
        <div className="field">
          ...
          <select
            ...
            onChange={(e) =>
              setForm({
                ...form,
                employmentType: e.target.value as Teacher["employmentType"],
              })
            }
          >
            {(
              Object.keys(EMPLOYMENT_KEYS) as (keyof typeof EMPLOYMENT_KEYS)[]
            ).map((p) => (
              <option key={p} value={p}>
                {t(EMPLOYMENT_KEYS[p])}
              </option>
